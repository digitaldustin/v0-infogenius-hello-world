...
// Using gemini-3-flash-preview for text/research and gemini-2.5-flash-image for visual generation/editing
...
/**
 * Extracts the first available image from a response's candidate parts.
 */
...
export const generateInfographicImage = async (prompt: string): Promise<string> => {
  ...
  // Fallback check: did the model return text explaining why it failed?
  const textResponse = response.text || "No image data returned from model.";
  throw new Error(`Generation error: ${textResponse}`);
};
...
export const fixInfographicImage = async (currentImageBase64: string, correctionPrompt: string): Promise<string> => {
  ...
  throw new Error("Failed to fix image: " + (response.text || "No image part found."));
};
...
export const editInfographicImage = async (currentImageBase64: string, editInstruction: string): Promise<string> => {
  ...
  throw new Error("Failed to edit image: " + (response.text || "No image part found."));
};
